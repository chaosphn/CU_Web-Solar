...
import { PeriodTime1 } from '../../models/period-time';
...
@Component({
  selector: 'app-sat-chart2',
  templateUrl: './sat-chart2.component.html',
  styleUrls: ['./sat-chart2.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SatChart2Component implements OnInit, OnDestroy, OnChanges {
  ...
  ngOnInit() {
    // const prd = this.dateTime.parseDate("t");
    // if(prd){
    //   this.period.startTime = prd.startTime;
    //   this.period.endTime = prd.endTime;
    // }
  ...
  setPositionTooltip(labelWidth, labelHeight, point) {
    ...
    const plotWidth = this.chartE2.nativeElement.clientWidth;
  ...
  initChartNew(_params: ChartParameters) {
    ...
    ////console.log(_config.getLegend1())
    ////console.log(_config.getSeries())
    let legendCheck: any = {}; 
    if(_config.getLegend1().enable){
      //console.log("1")
      legendCheck = _config.getLegend1();
    } else {
      //console.log("2")
      legendCheck = {
        enable: false
    ...
    const chart = new Chart({
      ...
      xAxis: {
        ...
        lineWidth: 0,
        min: new Date(new Date().setHours(0,0,0,0)).getTime() || null,
        max: new Date(new Date().setHours(23,59,0,0)).getTime() || null
      },
