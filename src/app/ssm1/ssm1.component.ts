...
@Component({
  selector: 'app-ssm1',
  templateUrl: './ssm1.component.html',
  styleUrls: ['./ssm1.component.scss'],
})
export class Ssm1Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  async init() {
    const req: string[] = await this.httpService.getConfig('assets/ssm1/tag-req.json');
    this.groupConfig = await this.httpService.getGroupConfig(req);
    this.reportConfig = await this.httpService.getConfig('assets/ssm1/report.config.json');
    this.initDateTime();
  }
