...
@Component({
  selector: 'app-ssm2',
  templateUrl: './ssm2.component.html',
  styleUrls: ['./ssm2.component.scss'],
})
export class Ssm2Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  async init() {
    const req: string[] = await this.httpService.getConfig('assets/ssm2/tag-req.json');
    this.groupConfig = await this.httpService.getGroupConfig(req);
    this.reportConfig = await this.httpService.getConfig('assets/ssm2/report.config.json');
    this.initDateTime();
  }
