...
@Component({
  selector: 'app-ssm3',
  templateUrl: './ssm3.component.html',
  styleUrls: ['./ssm3.component.scss'],
})
export class Ssm3Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  async init() {
    const req: string[] = await this.httpService.getConfig('assets/ssm3/tag-req.json');
    this.groupConfig = await this.httpService.getGroupConfig(req);
    this.reportConfig = await this.httpService.getConfig('assets/ssm3/report.config.json');
    this.initDateTime();
  }
