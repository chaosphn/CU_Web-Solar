...
@Component({
  selector: 'app-ssm4',
  templateUrl: './ssm4.component.html',
  styleUrls: ['./ssm4.component.scss'],
})
export class Ssm4Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  async init() {
    const req: string[] = await this.httpService.getConfig('assets/ssm4/tag-req.json');
    this.groupConfig = await this.httpService.getGroupConfig(req);
    this.reportConfig = await this.httpService.getConfig('assets/ssm4/report.config.json');
    this.initDateTime();
  }
