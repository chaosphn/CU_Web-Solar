...
@Component({
  selector: 'app-ssm5',
  templateUrl: './ssm5.component.html',
  styleUrls: ['./ssm5.component.scss'],
})
export class Ssm5Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  async init() {
    const req: string[] = await this.httpService.getConfig('assets/ssm5/tag-req.json');
    this.groupConfig = await this.httpService.getGroupConfig(req);
    this.reportConfig = await this.httpService.getConfig('assets/ssm5/report.config.json');
    this.initDateTime();
  }
