...
@Component({
  selector: 'app-ssm6',
  templateUrl: './ssm6.component.html',
  styleUrls: ['./ssm6.component.scss'],
})
export class Ssm6Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  async init() {
    const req: string[] = await this.httpService.getConfig('assets/ssm6/tag-req.json');
    this.groupConfig = await this.httpService.getGroupConfig(req);
    this.reportConfig = await this.httpService.getConfig('assets/ssm6/report.config.json');
    this.initDateTime();
  }
